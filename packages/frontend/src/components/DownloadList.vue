...
const commonDownload = async (
  ...
  try {
    ...
    link.target = '_blank'
    link.href = url
    link.download = file.includes('/') ? file.split('/').pop() || file : file
...
const downloadAudio = async (item: Audio, _: number) => {
  if (item.blobs) return downloadByBlobs(item.blobs, item.name || 'audio')
  if (!item.file) return
  await commonDownload(item, item.file, '音频', 'isDownloading')
}
const downloadSrt = async (item: Audio, _: number) => {
  console.log('item.srt', item.srt)
  if (!item.srt) return
  await commonDownload(item, item.srt, '字幕', 'isSrtLoading')
}
...
const downloadAll = async () => {
  for (const [index, item] of store.audioList.entries()) {
    if (!item.isDownloading) {
      await downloadAudio(item, index)
    }
  }
}
